...
class VectorStore:
    ...
    def get_course_outline(self, course_title: str) -> Optional[Dict[str, Any]]:
        """
        Return outline data for a course: title, link, and lesson list.
        Uses fuzzy resolution so partial names work.
        Returns None if course not found.
        """
        import json
        resolved = self._resolve_course_name(course_title)
        if not resolved:
            return None
        ...
        return {
            "title": meta.get("title", resolved),
            "course_link": meta.get("course_link"),
            "lessons": lessons   # list of {lesson_number, lesson_title, lesson_link}
        }
